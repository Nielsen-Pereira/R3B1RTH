...
import { SectionType, TR808Instrument } from '../../types/audio';

interface TR808SectionProps {
  section: SectionType;
}

const TR808Section: React.FC<TR808SectionProps> = ({ section }) => {
  const { tr808Params, setTR808Param, sectionParams, setSectionParam } = useAudioStore();
  const { patterns, currentPattern, patternLength, setStep, isPlaying } = useSequencerStore();
  const { theme } = useUIStore();

  const instruments: TR808Instrument[] = ['BD', 'SD', 'LT', 'MT', 'HT', 'RS', 'CP', 'CH', 'OH', 'CC', 'RC'];
  ...
  const sectionTitle = 'TR-808';
  const params = tr808Params;
          ...
          {instruments.map((instrument) => (
            <div key={instrument} className="instrument-control">
              <Button
                ...
                onClick={() => setTR808Param(instrument, 'level', params[instrument]?.level > 0 ? 0 : 100)}
              >
                {instrument}
              </Button>
              {'tune' in params[instrument] && (
                <Knob
                  ...
                  min={0}
                  max={100}
                  onChange={(v) => setTR808Param(instrument, 'tune', v)}
                  size="small"
                  label="T"
                />
              )}
              {'decay' in params[instrument] && (
                <Knob
                  ...
                  min={0}
                  max={100}
                  onChange={(v) => setTR808Param(instrument, 'decay', v)}
                  size="small"
                  label="D"
                />
              )}
              {instrument === 'SD' && 'tone' in params[instrument] && (
                <Knob
                  ...
                  min={0}
                  max={100}
                  onChange={(v) => setTR808Param(instrument, 'tone', v)}
                  size="small"
                  label="Tone"
                />
              )}
              {instrument === 'SD' && 'snap' in params[instrument] && (
                <Knob
                  ...
                  min={0}
                  max={100}
                  onChange={(v) => setTR808Param(instrument, 'snap', v)}
                  size="small"
                  label="Snap"
                />
              )}
...
export default React.memo(TR808Section);