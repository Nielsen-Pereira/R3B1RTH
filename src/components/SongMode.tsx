...
const SongMode: React.FC<SongModeProps> = ({ onClose }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => { nextPattern(currentSongId); }, 1000);
  ...
  const handleRenameSong = () => {
    ...
    if (newName) { renameSong(currentSongId, newName); }
  };
    ...
    return (
      <div className="song-mode-container">
        ...
        <div className="song-mode-empty">
          ...
          <button onClick={handleCreateSong} className="create-song-button">Create New Song</button>
        </div>
      </div>
    );
  }
    ...
    <div className="song-mode-container">
      <div className="song-mode-header">
        ...
        <div className="song-mode-actions">
          <button onClick={handleRenameSong} title="Rename Song">✏️</button>
          <button onClick={handleDeleteSong} title="Delete Song">🗑️</button>
          <button onClick={onClose} className="close-button">×</button>
        </div>
      </div>
      ...
      <div className="song-mode-controls">
        <div className="playback-controls">
          <button onClick={() => currentSongId && playSong(currentSongId)} disabled={!currentSong?.patterns.length} title="Play">▶️</button>
          <button onClick={() => currentSongId && stopSong(currentSongId)} disabled={!currentSong?.isPlaying} title="Stop">⏹️</button>
          <button onClick={() => currentSongId && previousPattern(currentSongId)} disabled={!currentSong?.isPlaying} title="Previous Pattern">⏮️</button>
          <button onClick={() => currentSongId && nextPattern(currentSongId)} disabled={!currentSong?.isPlaying} title="Next Pattern">⏭️</button>
      ...
      <div className="loop-controls">
        <label>
          <input type="checkbox" checked={currentSong?.loop.enabled || false} onChange={(e) => handleLoopChange(currentSong?.loop.start || 0, currentSong?.loop.end || 0, e.target.checked)} />
          Enable Loop
        </label>
        {currentSong?.loop.enabled && (
          <>
            <label>Start: <input type="number" min="0" max={currentSong.patterns.length - 1 || 0} value={currentSong.loop.start || 0} onChange={(e) => handleLoopChange(parseInt(e.target.value) || 0, currentSong.loop.end || 0, true)} /></label>
            <label>End: <input type="number" min="0" max={currentSong.patterns.length - 1 || 0} value={currentSong.loop.end || 0} onChange={(e) => handleLoopChange(currentSong.loop.start || 0, parseInt(e.target.value) || 0, true)} /></label>
          </>
        )}
      </div>
        ...
        {currentSong?.patterns.length === 0 ? (
          ...
          <ul>
            {currentSong?.patterns.map((pattern, index) => {
              ...
              return (
                <li key={pattern.id} draggable onDragStart={(e) => handleDragStart(e, index)} onDrop={(e) => handleDrop(e, index)} onDragOver={handleDragOver} className={classNames}>
                  <span className="pattern-index">{index + 1}.</span>
                  <span className="pattern-info">{pattern.section} - Pattern {String.fromCharCode(65 + Math.floor(pattern.patternIndex / 8))}{pattern.patternIndex % 8 + 1}</span>
                  <span className="pattern-length">{pattern.length} steps</span>
                  <button onClick={() => currentSongId && removePatternFromSong(currentSongId, pattern.id)} className="remove-pattern" title="Remove Pattern">×</button>
                </li>
              );
            })}
          </ul>
